    ...
    <div v-if="order">
      ...
      <section class="bg-white m-4 rounded-2xl p-4 shadow-sm">
        ...
        <div class="space-y-2 text-sm">
          ...
          <div class="flex justify-between">
            ...
            <span class="text-gray-800">¥{{ order.shipping_price }}</span>
          </div>
          <div class="flex justify-between">
          ...
          <div class="border-t pt-2 mt-2">
            <div class="flex justify-between">
              ...
              <span class="text-xl font-bold text-accent">¥{{ order.total_price + order.shipping_price - order.discount }}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="bg-white m-4 rounded-2xl p-4 shadow-sm">
        ...
        <div class="space-y-2 text-sm text-gray-500">
          ...
          <p>配送方式: {{ getShippingMethod(order.shipping_method) }}</p>
...
const order = computed(() => {
  return appStore.getOrderById(route.params.id)
})
...
const cancelOrder = async () => {
  if (confirm('确定要取消这个订单吗？')) {
    try {
      await appStore.cancelOrder(order.value.id)
    } catch (error) {
      alert('取消订单失败')
      console.error(error)
    }
