  ...
  <div class="min-h-screen bg-gray-50 pb-32">
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white border-t px-4 py-3 flex items-center justify-between max-w-3xl mx-auto">
      ...
      <button
        @click="submitOrder"
        :disabled="submitting"
        class="btn-primary px-8 py-3 rounded-full text-white font-bold shadow-lg disabled:opacity-50"
      >
        {{ submitting ? '提交中...' : '提交订单' }}
      </button>
    </div>
...
const discount = computed(() => {
  return totalPrice.value >= 199 ? 20 : 0
})

const finalPrice = computed(() => {
  return totalPrice.value + shippingPrice.value - discount.value
})
